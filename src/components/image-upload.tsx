...
export function ImageUpload({
  ...
  async function getMetadataForImages(
    files: File[]
  ...
  return (
    <div className="space-y-6">
      ...
      {!boardId.length && <p className="text-red-500 text-sm">{message}</p>}
      {textGenerationLoading && (
        <div className="bg-[rgb(0,0,0,0.9)] h-[100vh] w-[100vw] fixed top-0 left-0 z-10 flex items-center justify-center">
          <div>
            <Spinner size="medium" className="text-white m-3" />
            <p className="text-md text-white">
              Generating Title and Description for uploaded assets
            </p>
          </div>
        </div>
      )}
